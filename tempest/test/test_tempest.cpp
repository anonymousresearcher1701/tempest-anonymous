...
protected:
    void SetUp() override {
        tempest = std::make_unique<Tempest>(true, T::value, -1, true, -1);
    }
...
protected:
    void SetUp() override {
        tempest = std::make_unique<Tempest>(true, T::value, MAX_TIME_CAPACITY, true, -1);
    }
...
protected:
    ...
    void SetUp() override {
        tempest = std::make_unique<Tempest>(false, T::value, -1, true, -1);
        tempest->add_multiple_edges(sample_edges);
    }
...
protected:
    void SetUp() override {
        tempest = std::make_unique<Tempest>(true, T::value, -1, true, 10.0);
...
// Test the constructor of Tempest to ensure it initializes correctly.
TYPED_TEST(EmptyTempestTest, ConstructorTest) {
    EXPECT_NO_THROW(this->tempest = std::make_unique<Tempest>(true, TypeParam::value));
    EXPECT_EQ(this->tempest->get_node_count(), 0); // Assuming initial node count is 0
}


// Test adding an edge to the Tempest when it's empty.
...
// Test to check if a specific node ID is present in the filled Tempest.
