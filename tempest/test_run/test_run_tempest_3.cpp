...
void benchmark_exponential_index_forward(const std::vector<int> &sources,
    ...
    const Tempest tempest(false, use_gpu, -1, false, timescale_bound);
    ...
    std::cout << "Edge count after insertion: " << tempest.get_edge_count() << std::endl;
    ...
    const auto walks = tempest.get_random_walks_and_times_for_all_nodes(
...
void benchmark_exponential_weight_forward(const std::vector<int> &sources,
    ...
    const Tempest tempest(false, use_gpu, -1, true, timescale_bound);
